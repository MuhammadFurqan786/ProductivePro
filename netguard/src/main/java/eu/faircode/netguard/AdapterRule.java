...
public class AdapterRule extends RecyclerView.Adapter<AdapterRule.ViewHolder> implements Filterable {
    ...
    public static class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public ViewHolder(View itemView) {
            ...
            final View wifiParent1 = (View) sStatus.getParent();
            wifiParent.post(new Runnable() {
                public void run() {
                    Rect rect = new Rect();
                    sStatus.getHitRect(rect);
                    rect.bottom += rect.top;
                    rect.right += rect.left;
                    rect.top = 0;
                    rect.left = 0;
                    wifiParent1.setTouchDelegate(new TouchDelegate(rect, sStatus));
                }
            });
    ...
    public AdapterRule(Context context, View anchor) {
        ...
        if (prefs.getBoolean("dark_theme", false))
            ...
            colorChanged = Color.argb(128, Color.red(R.color.colorTealAccent), Color.green(R.color.colorTealAccent), Color.blue(R.color.colorTealAccent));
...
//        holder.sStatus.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
//            @Override
...
//        holder.tvName.setTextColor(color);
        ...
        holder.sStatus.setEnabled(rule.apply);
        holder.sStatus.setAlpha(wifiActive ? 1 : 0.5f);
        holder.sStatus.setOnCheckedChangeListener(null);
        holder.sStatus.setChecked(rule.wifi_blocked);
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
            Drawable wrap = DrawableCompat.wrap(CompoundButtonCompat.getButtonDrawable(holder.sStatus));
            DrawableCompat.setTint(wrap, rule.apply ? (rule.wifi_blocked ? colorOff : colorOn) : colorGrayed);
        }
        holder.sStatus.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            ...
            public void onClick(View view) {
                Util.areYouSure(view.getContext(), R.string.msg_clear_rules, new Util.DoubtListener() {
                    ...
                    public void onSure() {
                        ...
                        holder.sStatus.setChecked(rule.wifi_default);
    ...
    private void showdialogue(final Context context, final Rule rule) {
        ...
        dialog.setContentView(R.layout.freez_dialog);
        dialog.getWindow().setLayout((int) (width * 0.8), (int) (height * 0.62)); //Controlling width and height.
        dialog.getWindow().setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));

        final SwipeSelector hours_selector = dialog.findViewById(R.id.hours_selector);
        ImageView imgclose = dialog.findViewById(R.id.imgclose);
        Button sendButton = dialog.findViewById(R.id.sendButton);
        hours_selector.setItems(
                // The first argument is the value for that item, and should in most cases be unique for the
                // current SwipeSelector, just as you would assign values to radio buttons.
                // You can use the value later on to check what the selected item was.
                // The value can be any Object, here we're using ints.
                new SwipeItem(0, "30 Minutes", "Just a tiny bit of control, lock my apps for 30 minutes"),
                new SwipeItem(1, "1 Hour", "Quick and fast, lock my apps for 1 hour"),
                new SwipeItem(2, "2 Hours", "I need to finish a task that needs a bit of time, lock my apps for 2 hours"),
                new SwipeItem(2, "3 Hours", "lock my apps for 3 hours")
        );
        ...
        sendButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Toast.makeText(context, "" + hours_selector.getSelectedItem().title.toLowerCase(), Toast.LENGTH_SHORT).show();
